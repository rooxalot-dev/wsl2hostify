...
#[REG KEY ON WINDOWS]
$registryPath = 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\App Paths\wsl2hostify.exe';

if (!(Test-Path $registryPath)) {
	Write-Output "--> 'wsl2hostify' key registering on Windows...`n";	
	...
	Write-Output "--> 'wsl2hostify' key already exist...`n";
}

#[SHEDULE TASK ON WINDOWS]
$foundTask = Get-ScheduledTask -TaskName 'wsl2hostify' -EA SilentlyContinue;
...
if (!$foundTask) {
    ...
    Register-ScheduledTask -TaskName "wsl2hostify" -InputObject $task | Out-Null;	
} else {
...
if ($foundRemoteAddress) {
	...
	Write-Output "The WSDL address: $remoteAddress"
	Write-Output "The Local address: $localAddress"
} else {
